...
class AbstractInstanceParser {
            ...
            for (auto i = 0; i < matrix_size - 1; i++) {
             ...
             for(auto i = 0; i < matrix_size; i++) {
                    for(auto j = 0; j < matrix_size; j++) {
                        costs_matrix.at(i, j, 0);
                    }
                }

                for(auto idx = 0; idx < explicit_costs_i.size(); idx++) {
                    const auto i = explicit_costs_i[idx];
                    const auto j = explicit_costs_j[idx];
                    const auto cost = explicit_costs_cost[idx];
                    
                    costs_matrix.at(i, j, cost);
                    costs_matrix.at(j, i, cost);
                }
        }
 ...
 protected:
    ...
    std::vector<int> explicit_costs_i;
    std::vector<int> explicit_costs_j;
    std::vector<float> explicit_costs_cost;
...
template <bool round_costs>
class EInstanceParser : public AbstractInstanceParser<round_costs> {

    public:

    explicit EInstanceParser(const std::string& path_) : AbstractInstanceParser<round_costs>(path_){ }
    ...
    bool parse_impl() {
        ...
        try {
            ...
            while(true) {
                ...
                this->explicit_costs_i.emplace_back(static_cast<int>(i)-1);
                this->explicit_costs_j.emplace_back(static_cast<int>(j)-1);
                this->explicit_costs_cost.emplace_back(cost);
